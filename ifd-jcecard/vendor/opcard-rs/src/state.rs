...
impl ArbitraryDO {
    ...
    fn default(self) -> Bytes<MAX_GENERIC_LENGTH> {
        ...
        match self {
            ...
            //
            // Per OpenPGP card v3.4 §4.4.3.9 the GET DATA response for DO
            // 0x00F9 must contain only the inner TLVs, not a wrapping
            // `F9 03 ...` header. Upstream opcard-rs v1.7.0 returns a
            // double-tagged `F9 03 81 01 00` by default; scdaemon
            // misinterprets that as "KDF algorithm unknown / KDF-salted
            // mode enabled", pads the PIN to 32 bytes with ASCII '@'
            // (0x40), and every subsequent VERIFY fails with "Bad PIN".
            //
            // Patched locally to emit just the inner `81 01 00`
            // (algorithm = none). See `ifd-jcecard/vendor/opcard-rs/`
            // for the vendor snapshot; report upstream when convenient.
            Self::KdfDo => Bytes::from(&hex!("81 01 00")),
            _ => Bytes::new(),
        }
    }

...
impl<R: Runner> App<R> for OpcardApp<R> {
    ...
    fn with_client(runner: &R, trussed: Client<R>, _: (), config: &OpcardConfig) -> Self {
        ...
        {
            // Upstream restricts allowed_generation to P_256 + RSA_2048 +
            // X_25519 + Ed_25519 on non-se050 builds because real NK3
            // hardware delegates P-384 / P-521 to the SE050 secure
            // element. In our host build we enable trussed/p384 +
            // trussed/p521 so the software backends are present, so we
            // also widen allowed_{imports,generation} to include them.
